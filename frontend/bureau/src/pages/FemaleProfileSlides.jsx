...
const FemaleProfileSlides = () => {
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      try {
        const response = await apiClient.get(`${apiEndpoints.fetchbureau}/${bureauId}/female`);
  ...
  // Measure card width and visible count for responsive slider
  useEffect(() => {
    ...
    <div className="bg-gray-100 p-4 rounded-lg relative">
      ...
      <div className="flex items-center justify-between mb-1">
        <h1 className="text-lg sm:text-3xl font-bold text-gray-800 tracking-tight leading-snug flex items-center mb-2">
          <span className="text-purple-700">{t("ownfemaleprofiles")}</span>
          {/* <span className="text-gray-500 font-medium ml-1 sm:ml-2">(&nbsp;Female&nbsp;)</span> */}
        </h1>
        <h1>
...
export default FemaleProfileSlides;
