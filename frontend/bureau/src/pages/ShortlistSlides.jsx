    ...
    const fetchProfiles = async () => {
      try {
        ...
        if (response.data?.success && response.data?.data) {
          shortlistItems = Array.isArray(response.data.data)
            ? response.data.data
            : [response.data.data];
        ...
        const processed = await Promise.all(
          shortlistItems.slice(0, 10).map(async (item) => {
            const profileId = item.profileId || item.userId || item._id;
            try {
              const res = await apiClient.get(`${apiEndpoints.user}/${profileId}`);
              const data = res.data?.data || res.data;
              return {
                ...data,
                shortlistId: item._id,
                isShortlisted: true,
                visibleToAll: convertStringToBoolean(data.visibleToAll),
              };
            } catch {
              return {
                _id: profileId,
        ...
        setTotalCount(shortlistItems.length);
      } catch (err) {
    ...
    <div className="bg-gray-100 p-4 rounded-lg relative">
      ...
      <div className="flex items-center justify-between mb-1">
        <h1 className="text-lg sm:text-3xl font-bold text-gray-800 tracking-tight leading-snug flex items-center mb-2">
          <span className="text-purple-700">{t("shortlist")}</span>
        </h1>
        <h1>
...
export default ShortlistSlides;
