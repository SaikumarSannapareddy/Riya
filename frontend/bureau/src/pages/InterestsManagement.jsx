...
import { FaArrowLeft, FaHeart, FaInbox, FaPaperPlane, FaCheck, FaTimes, FaEye, FaEyeSlash, FaClock, FaUser, FaSearch, FaFilter, FaBell, FaPhone, FaEnvelope, FaMapMarkerAlt, FaGraduationCap, FaBriefcase } from 'react-icons/fa';
...
const InterestsManagement = () => {
  ...
  const fetchNotificationCount = async () => {
    try {
      const response = await apiClient.post(apiEndpoints.getNotificationCount, {
        bureauId
      });
  ...
  const updateInterestStatus = async (interestId, newStatus) => {
    ...
    try {
      ...
      if (response.data.success) {
        // Update the interest in the local state
  ...
  const markAllAsSeen = async () => {
    try {
      const response = await apiClient.post(apiEndpoints.markAllNotificationsSeen, {
        bureauId
      });

      if (response.data.success) {
        // Update all pending interests to seen
        setReceivedInterests(prev => 
          prev.map(interest => 
  ...
  const getFilteredInterests = (interests) => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter(interest => 
        interest.martialId?.toLowerCase().includes(term) ||
        interest.description?.toLowerCase().includes(term) ||
        interest.senderbureauId?.toLowerCase().includes(term) ||
        interest.targetbureauId?.toLowerCase().includes(term)
      );
    }
  ...
  const handleViewDetail = async (interest) => {
    ...
    try {
      ...
      // For received interests: sender is the one who sent the interest (martialId)
      if (activeTab === 'received') {
        if (interest.martialId) {
          promises.push(
            apiClient.get(`/api/martial/${interest.martialId}`)
        ...
        // For sent interests: you are the sender (martialId), target is the receiver (targetmartialId)
        if (interest.martialId) {
          promises.push(
            apiClient.get(`/api/martial/${interest.martialId}`)
              .then(response => ({ type: 'sender', data: response.data }))
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              ...
              <h1 className="text-2xl font-bold text-gray-900">Other Bureau Interests History</h1>
            </div>
                ...
                >
                  ...
                  <div className="flex items-center mb-4 p-3 bg-blue-50 rounded-xl shadow-sm">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-600 mb-1">Martial ID: {interest.senderProfile?.martialId || interest.martialId}</div>
                      ...
                      <div className="flex items-center space-x-3 mb-3">
                        ...
                        <div>
                          <div className="flex-1 mb-4 md:mb-0">
                            <div className="flex items-center mb-2">
                              ...
                              <h3 className="text-xl font-bold text-gray-800">
                                {activeTab === 'received' ? (
                                  <>
                                    <span className="text-gray-500">I am Interested in your Profile </span> {interest.martialId}
                                  </>
                                ) : (
                                  <>
                                    <span className="text-gray-500">Sent request to: </span> {interest.martialId}
                                  </>
                                )}
                              </h3>
                            </div>
                          ...
                          <p className="text-sm text-gray-500">
                            {activeTab === 'received' 
                              ? `From Bureau: ${interest.senderbureauId}`
                              : `To Bureau: ${interest.targetbureauId}`
                            }
                          </p>
                        </div>
                      </div>
                          ...
                          {activeTab === 'received' && interest.status === 'pending' && (
                            <div className="flex items-center space-x-2">
                              <button
                                onClick={() => updateInterestStatus(interest._id, 'accepted')}
                                disabled={updatingStatus}
                                className="inline-flex items-center px-3 py-1.5 border border-transparent rounded-md text-sm font-medium text-white bg-green-600 hover:bg-green-700 disabled:opacity-50 transition-colors"
                              >
                              ...
                              <button
                                onClick={() => updateInterestStatus(interest._id, 'rejected')}
                                disabled={updatingStatus}
                                className="inline-flex items-center px-3 py-1.5 border border-transparent rounded-md text-sm font-medium text-white bg-red-600 hover:bg-red-700 disabled:opacity-50 transition-colors"
                              >
            ...
            <div className="p-6">
              ...
              <div className="bg-blue-50 rounded-lg p-4 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      {activeTab === 'received' ? 'From Bureau' : 'To Bureau'}
                    </label>
                    <p className="text-gray-900">
                      {activeTab === 'received' 
                        ? selectedInterest.senderbureauId 
                        : selectedInterest.targetbureauId
                      }
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-gray-900">{selectedInterest.martialId}</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                    ...
                    {activeTab === 'received' ? 'Sender Profile' : 'Your Profile (Sender)'}
                  </h3>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                    ...
                    {activeTab === 'received' ? 'Your Profile (Target)' : 'Target Profile'}
                  </h3>
