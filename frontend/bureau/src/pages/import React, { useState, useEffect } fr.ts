...
const Apartment_Flat_Details = () => {
  ...
  const dropdownOptions = {
    Number_Of_Plots: [
      { value: "", label: "Select Number", isDisabled: true }, // Disabled option as a placeholder
    ...
    Flat_Type: [
      { value: "", label: "Select Flat Type", isDisabled: true }, // Disabled option as a placeholder
      ...Array.from({ length: 10 }, (_, i) => ({
        value: `${i + 1}bhk`,
        label: `${i + 1} BHK`,
      })),
    ...
    Agriculture_Land: [
      { value: "", label: "Select Land Area (in Acres)", isDisabled: true }, // Disabled option as a placeholder
    ...
    Anymore_Properties: [
      { value: "", label: "Select", isDisabled: true }, // Disabled option as a placeholder
      { value: "yes", label: "Yes" },
      { value: "no", label: "No" },
    ],
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedOptions = {
      ...
      numberOfFlats: selectedOptions.numberOfFlats?.value || '',
      flatType: selectedOptions.flatType?.value || '',
      flatValue: parseFloat(selectedOptions.flatValue?.value) || 0,
      agricultureLand: parseInt(selectedOptions.agricultureLand?.value) || 0,
      agricultureLandValue: parseFloat(selectedOptions.agricultureLandValue?.value) || 0,
      anyMoreProperties: selectedOptions.anyMoreProperties?.value || "no",
      totalPropertiesValue: parseFloat(selectedOptions.totalPropertiesValue?.value) || 0,
      propertyNames: selectedOptions.propertyNames?.value || "",
    };

    const formDataToSend = {
      // Updated flatLocation to be just an array of strings (city names)
      ...formattedOptions,
      flatLocation: selectedOptions2.Flat_Location?.value || '',
      agricultureLandLocation: selectedOptions3.Agriculture_Land_Location?.value || '',
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-blue-50 to-indigo-100">
      <div className="w-full max-w-lg bg-white shadow-lg rounded-lg">
        ...
        {/* Form */}
        <form onSubmit={handleSubmit} className="space-y-6 mt-12 px-3 py-4">
