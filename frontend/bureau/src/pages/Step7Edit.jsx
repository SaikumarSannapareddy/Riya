...
const Apartment_Flat_Details = () => {
  ...
  const dropdownOptions = {
    ...
    Flat_Value: [
      ...
      ...Array.from({ length: 200 }, (_, index) => ({
        value: 50000 + index * 50000,
        label: `₹${(50000 + index * 50000).toLocaleString("en-IN")}`,
      })),
    ],
    Agriculture_Land: [
    ...
    Agriculture_Land_Value: [
     // Disabled option as a placeholder
      ...Array.from({ length: 2000 }, (_, index) => {
        const value = 100000 + index * 100000;
        return {
          value,
          label: `₹${value.toLocaleString("en-IN")}`,
        };
      }),
      { value: 10000000000, label: "₹100,00,00,000" }, // Adding ₹100 crore as the final value
    ],
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedOptions = {
      ...
      openPlots: selectedOptions.Number_Of_Plots?.value || '',
    ...
    const fetchData = async () => {
      try {
        ...
        if (response?.status === 200 && response?.data) {
          ...
          setSelectedOptions2({
            Flat_Location: Array.isArray(data.flatLocation) ? data.flatLocation : [],
          });
          setSelectedOptions3({
          ...
          setSelectedOptions((prevState) => ({
            Number_Of_Plots: dropdownOptions?.Number_Of_Plots?.find(option => option.value === data.openPlots) || prevState.Number_Of_Plots,
            Flat_Type: dropdownOptions?.Flat_Type?.find(option => option.value === data.flatType) || prevState.Flat_Type,
            Flat_Value: dropdownOptions?.Flat_Value?.find(option => option.value === data.flatValue) || prevState.Flat_Value,
            Agriculture_Land: dropdownOptions?.Agriculture_Land?.find(option => option.value === data.agricultureLand) || prevState.Agriculture_Land,
            Agriculture_Land_Value: dropdownOptions?.Agriculture_Land_Value?.find(option => option.value === data.agricultureLandValue) || prevState.Agriculture_Land_Value,
            Total_Properties_Value: dropdownOptions?.Total_Properties_Value?.find(option => option.value === data.totalPropertiesValue) || prevState.Total_Properties_Value,
            Anymore_Properties: dropdownOptions?.Anymore_Properties?.find(option => option.value === data.anyMoreProperties) || prevState.Anymore_Properties,
          }));
        } else {
