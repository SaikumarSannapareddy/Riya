...
const MalePendingSlides = () => {
 ...
 useEffect(() => {
    const fetchProfiles = async () => {
      try {
        const response = await apiClient.get(`${apiEndpoints.fetchpending}/${bureauId}/male`);
    ...
    <div className="bg-gray-100 p-4 rounded-lg relative"> 
       ...
       <div className="flex items-center justify-between mb-1">
        <h1 className="text-lg sm:text-3xl font-bold text-gray-800 tracking-tight leading-snug flex items-center mb-2">
          <span className="text-purple-700">{t("malepending")}</span>
        </h1>
        <h1>
      ...
      <div className="relative overflow-hidden" ref={containerRef}>
        ...
        >
          {profiles.map((profile, idx) => (
            <div
              key={profile._id}
 ref={idx === 0 ? cardRef : null} className="card flex-shrink-0 cursor-pointer"            
...
export default MalePendingSlides;
