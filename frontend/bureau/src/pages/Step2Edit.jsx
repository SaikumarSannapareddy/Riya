...
const Personal_Details_Form = () => {
      ...
      const fetchData = async () => {
        try {
          ...
          if (response.status === 200 && response.data) {
            ...
            setSelectedOptions({
              height: { value: data.height, label: `${data.height} ft` },
              weight: { value: data.weight, label: `${data.weight} kg` },
              Physical_State: dropdownOptions.Physical_State.find(option => option.value === data.physicalState) || { value: "", label: "Select" },
              Eating_Habits: dropdownOptions.Eating_Habits.find(option => option.value === data.eatingHabits) || { value: "", label: "Select" },
              Smoking_Habits: dropdownOptions.Smoking_Habits.find(option => option.value === data.smokingHabits) || { value: "", label: "Select" },
              Drinking_Habits: dropdownOptions.Drinking_Habits.find(option => option.value === data.drinkingHabits) || { value: "", label: "Select" },
            });
          } else {
  ...
  const [selectedOptions, setSelectedOptions] = useState({
    height: { value: "3.1", label: "3.1 ft" }, // Default value for height
    weight: { value: "30", label: "30 kg" }, // Default value for weight
    Physical_State: { value: "", label: "Select" }, // Default for physical state
    Eating_Habits: { value: "", label: "Select" }, // Default for eating habits
    Smoking_Habits: { value: "", label: "Select" }, // Default for smoking habits
    Drinking_Habits: { value: "", label: "Select" }, // Default for drinking habits
  });

  const dropdownOptions = {
    height: Array.from({ length: 41 }, (_, i) => {
      const value = (3.1 + i * 0.1).toFixed(1); // Generate heights from 3.1 to 7.4
    ...
    >
      ...
      <div className="flex items-center justify-between ">
        ...
        <RiArrowRightSLine className="text-3xl text-gray-500 ml-3" />
          ...
          <button
            type="submit"
            className="w-full bg-red-700 hover:bg-red-500 text-white font-bold py-3 rounded-lg transition-colors"
          >
