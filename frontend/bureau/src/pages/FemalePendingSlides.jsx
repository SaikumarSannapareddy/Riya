...
const FemalePendingSlides = () => {
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      try {
        const response = await apiClient.get(`${apiEndpoints.fetchpending}/${bureauId}/female`);
        ...
        if (Array.isArray(data.users)) {
          ...
          setTotalCount(data.totalPending ?? processed.length);
        } else {
          setProfiles([]);
          setTotalCount(0);
        }
    ...
    <div className="bg-gray-100 p-4 rounded-lg relative">
      ...
      <div className="flex items-center justify-between mb-1">
        <h1 className="text-lg sm:text-3xl font-bold text-gray-800 tracking-tight leading-snug flex items-center mb-2">
          <span className="text-purple-700"> {t("femalepending")}</span>
        </h1>
        <h1>
...
export default FemalePendingSlides;
