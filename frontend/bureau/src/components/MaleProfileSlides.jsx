...
const MaleProfileSlides = () => {
  ...
  const cardRef = useRef(null); // reference for measuring one card
      ...
      try {
        ...
        if (Array.isArray(data.users)) {
          const processed = data.users
            .map((p) => ({ ...p, visibleToAll: convertStringToBoolean(p.visibleToAll) }))
            .slice(0, 10);

          setProfiles(processed);
          setTotalCount(data.total ?? processed.length);
        } else {
          setError('Invalid response format. Users array missing.');
        }
      } catch (err) {
  ...
  // Measure card width and visible count using CSS-defined widths (media queries handle the widths)
  useEffect(() => {
    const updateMeasurements = () => {
      ...
      // how many cards fit fully in the container
      const visible = Math.max(1, Math.floor(container.clientWidth / cardW));

      setCardWidthPx(cardW);
      setVisibleCount(visible);

      // adjust currentIndex if it went out of range after resize
    ...
    // also update after images load (in case images change sizes)
  ...
  <h1 className="text-lg sm:text-3xl font-bold text-gray-800 tracking-tight leading-snug flex items-center mb-2">
    <span className="text-purple-700">{t("ownmaleprofiles")}</span>
    {/* <span className="text-gray-500 font-medium ml-1 sm:ml-2">(&nbsp;Male&nbsp;)</span> */}
      </h1>
    <h1>
      ...
      <div className="relative overflow-hidden" ref={containerRef}>
        <div
          ...
          style={{
            transform: `translateX(-${currentIndex * cardWidthPx}px)`,
          }}
        >
          {profiles.map((profile, idx) => (
            <div
              key={profile._id}
              // attach ref only to first card for measurement
            ...
            >
              <div className="relative inline-block w-full">
                <img
                  src={
                    profile.image ? `${Uploads}${profile.image}` : 'https://www.pngitem.com/pimgs/m/581-5813504_avatar-dummy-png-transparent-png.png'
                  }
                  alt={profile.fullName}
                  className="w-full h-48 object-cover rounded-lg shadow-md"
                />
                {/* <span className="absolute top-2 right-2 bg-blue-100 text-blue-800 text-sm font-bold px-2 py-1 rounded-full shadow-lg">
  ...
  className="flex items-center justify-center w-full px-10 py-1 border-2 border-purple-600 rounded-full text-purple-600 font-medium hover:bg-purple-600 hover:text-white transition-all duration-300 group"
>
  <span>{t("viewall")}</span>
