// Advanced Theme System with Dark Colors and Auto-Updating
...
export const darkButtonClasses = {
  ...
  outline: `${darkThemeClasses.borderPrimary} ${darkThemeClasses.borderGray} text-gray-700 hover:bg-gray-50 transition duration-300`,
};
...
const ThemeContext = createContext({
  ...
  buttonClasses: darkButtonClasses,
  updateTheme: (newColors: Partial<typeof darkThemeColors>) => {},
});
...
export const ThemeProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const updateTheme = (newColors: Partial<typeof darkThemeColors>) => {
    const updatedColors = { ...theme.colors, ...newColors };
    setTheme({
      colors: updatedColors,
      classes: darkThemeClasses, // You can also update classes if needed
      buttonClasses: darkButtonClasses,
    });
    
    // Dispatch custom event for non-React components
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('themeUpdated', { detail: updatedColors }));
    }
  };
...
// Auto-updating theme system for non-React components
export const themeSystem = {
  ...
  updateTheme: (newColors: Partial<typeof darkThemeColors>) => {
    Object.assign(darkThemeColors, newColors);
    // Trigger re-render for all components using this theme
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('themeUpdated', { detail: darkThemeColors }));
    }
  },
  
  // Subscribe to theme changes
  subscribe: (callback: (colors: typeof darkThemeColors) => void) => {
    if (typeof window !== 'undefined') {
  ...
  unsubscribe: (callback: (colors: typeof darkThemeColors) => void) => {
    if (typeof window !== 'undefined') {
...
export const themeClasses = darkThemeClasses;
export const buttonClasses = darkButtonClasses;
