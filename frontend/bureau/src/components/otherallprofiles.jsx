...
import { FaArrowLeft, FaUser, FaEye, FaEdit, FaTrash, FaShareAlt, FaSearch, FaEllipsisV } from 'react-icons/fa';
...
const ProfilePage = () => {
  ...
  const profilesPerPage = 15; // Profiles to show per page
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      if (bureauId) {
        try {
          const response = await apiClient.get(`${apiEndpoints.fetchotherbureaumain}/${bureauId}`);
  ...
  const handleNavigationClick = (gender) => {
    ...
    if (gender === 'All') {
      ...
      path = gender === 'male' ? 'other-male-profiles' : 'other-female-profiles';
    }
    ...
    <div className="min-h-screen p-4 bg-gray-100 text-gray-800">
     <div className="sticky top-0 bg-gray-100 z-10 py-4 mb-4">
        <div className="flex items-center mb-4">
          ...
          />
          <h1 className="text-2xl font-semibold">Other Profiles</h1>
        </div>
          ...
          {currentProfiles.map((profile) => (
            ...
            >
              ...
              <div className="flex justify-around items-center mt-4 text-gray-800 space-x-6">
                ...
                <div className="text-center">
                  <FaEllipsisV className="text-gray-500 mb-1" size={20} />
