...
const LocationsManagement = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        await apiClient.put(`${apiEndpoints.locationlinksupdate}`, videoData);
      } else {
        await apiClient.post(apiEndpoints.locationlinksAdding, videoData);
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      await apiClient.delete(`${apiEndpoints.locationlinksdelete}/${id}`);
      fetchVideos();
    } catch (err) {
  ...
  return (
      ...
      <h1 className="text-3xl font-bold text-gray-900 mb-8">Locations Management</h1>
      ...
      <div className="bg-white shadow-md rounded-lg p-6 mb-8">
        <h2 className="text-xl font-semibold mb-4">
          {isEditing ? "Edit Location" : "Add New Loaction"}
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleInputChange}
              placeholder="https://www.mapgoogle.com/watch?v=..."
              className="w-full px-4 py-2 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200 ease-in-out text-sm"
            />
          </div>
          ...
          <div className="flex justify-end space-x-3">
            ...
            >
              {loading ? "Processing..." : isEditing ? "Update Location" : "Add Location"}
            </button>
          </div>
        </form>
      </div>
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        <h2 className="text-xl font-semibold p-6 border-b">All Locations</h2>
        ...
        {loading && !videos.length ? (
          ...
          <div className="text-center py-12 text-gray-500">
            No Locations found. Add your first one above.
          </div>
        ) : (
