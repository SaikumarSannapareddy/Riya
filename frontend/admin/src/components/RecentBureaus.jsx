...
const RecentBureaus = () => {
  ...
  const fetchBureaus = async () => {
    setLoading(true);
    try {
      const response = await apiClient.get(apiEndpoints.BureauRecent);
      ...
      // Exclude deleted/suspended
      const recent = data.bureauProfiles.filter(bureau => bureau.deleted != 1 && bureau.suspend != 1);

      setBureaus(recent);
      filterAndPaginateBureaus(recent, searchQuery, currentPage);
    } catch (error) {
      console.error("Error fetching recent bureau profiles:", error);
      setBureaus([]);
      setFilteredBureaus([]);
      setTotalPages(0);
    } finally {
  ...
  const formatDate = (dateString) => {
    ...
    // createdAt is in 'YYYY-MM-DD HH:MM:SS'
    const dt = new Date(dateString);
    return isNaN(dt.getTime()) ? dateString : dt.toLocaleDateString();
  };

  const getBureauStatusInfo = (bureau) => {
    if (bureau.deleted == 1) return { class: "bg-red-500", text: "Deleted" };
    if (bureau.suspend == 1) return { class: "bg-orange-500", text: "Suspended" };
    return { class: "bg-blue-500", text: "Recent" };
  };

  return (
    <div className="container mx-auto px-4 py-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <h1 className="text-2xl font-bold mb-4 sm:mb-0">Recent Bureaus (Last 7 days)</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {filteredBureaus.length > 0 ? (
              filteredBureaus.map((bureau) => {
                ...
                return (
                  <div key={bureau.bureauId} className={`border rounded-lg shadow-md p-4 bg-white flex flex-col relative hover:shadow-lg transition-shadow duration-300 ${bureau.deleted == 1 ? 'border-red-300 bg-red-50' : bureau.suspend == 1 ? 'border-orange-300 bg-orange-50' : ''}`}>
                    <div className={`absolute top-3 left-3 ${statusInfo.class} text-white text-xs px-2 py-1 rounded-full`}>{statusInfo.text}</div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center sm:items-start mt-6">
                      ...
                      <div className="flex-1 w-full">
                        ...
                        <div className="mb-2 flex items-start"><AiOutlineUser className="text-gray-600 mr-2 mt-1 flex-shrink-0" /><div className="min-w-0"><h3 className="font-bold text-sm text-gray-800">Owner Name</h3><p className="text-gray-600 truncate">{bureau.ownerName}</p></div></div>
                    ...
                    {/* Action Buttons (same as FreeBureaus without payment modal) */}
                    <div className="mt-4 flex flex-wrap gap-2 justify-between">
                      <button className={`flex-1 ${bureau.deleted == 1 ? 'bg-gray-400' : 'bg-blue-500 hover:bg-blue-600'} text-white px-2 py-1.5 rounded flex items-center justify-center transition-colors text-sm`} disabled={bureau.deleted == 1}>
                        <AiOutlineEye className="mr-1" /> View
                      </button>
                      <button className={`flex-1 ${bureau.deleted == 1 ? 'bg-gray-400' : 'bg-yellow-500 hover:bg-yellow-600'} text-white px-2 py-1.5 rounded flex items-center justify-center transition-colors text-sm`} disabled={bureau.deleted == 1}>
                        <AiOutlineEdit className="mr-1" /> Edit
                      </button>
                      {bureau.deleted == 1 ? (
                        <button className="flex-1 bg-green-500 text-white px-2 py-1.5 rounded flex items-center justify-center hover:bg-green-600 transition-colors text-sm">
                          <AiOutlineEye className="mr-1" /> Restore
                        </button>
                      ) : (
                        <button className="flex-1 bg-red-500 text-white px-2 py-1.5 rounded flex items-center justify-center hover:bg-red-600 transition-colors text-sm">
              ...
              <div className="col-span-full text-center py-8"><p className="text-gray-500 text-lg">No recent bureaus found.</p></div>
            )}
          </div>
