...
const ExpiredBureaus = () => {
  ...
  const fetchBureaus = async () => {
    setLoading(true);
    try {
      const response = await apiClient.get(apiEndpoints.BureauExpired);
      const data = response.data;
      const list = Array.isArray(data?.bureauProfiles) ? data.bureauProfiles : [];
      // Exclude deleted if desired; keep suspended viewable
      const expired = list.filter(b => b.deleted != 1);
      setBureaus(expired);
      filterAndPaginateBureaus(expired, searchQuery, currentPage);
    } catch (error) {
      console.error("Error fetching expired bureaus:", error);
      setBureaus([]);
      setFilteredBureaus([]);
      setTotalPages(0);
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <h1 className="text-2xl font-bold mb-4 sm:mb-0">Expired Bureaus</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {filteredBureaus.length > 0 ? (
              filteredBureaus.map((bureau) => (
                <div key={bureau.bureauId} className={`border rounded-lg shadow-md p-4 bg-white flex flex-col relative hover:shadow-lg transition-shadow duration-300`}>
                  <div className={`absolute top-3 left-3 bg-gray-700 text-white text-xs px-2 py-1 rounded-full`}>Expired</div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center sm:items-start mt-6">
                    ...
                    <div className="flex-1 w-full">
                      ...
                      <div className="mb-2 flex items-start"><AiOutlineCalendar className="text-gray-600 mr-2 mt-1 flex-shrink-0" /><div className="min-w-0"><h3 className="font-bold text-sm text-gray-800">Expiry Date</h3><p className="text-gray-600 truncate">{formatDate(bureau.expiryDate)}</p></div></div>
                    </div>
                  </div>

                  <div className="mt-4 flex flex-wrap gap-2 justify-between">
                    <button className={`flex-1 bg-blue-500 hover:bg-blue-600 text-white px-2 py-1.5 rounded flex items-center justify-center transition-colors text-sm`}>
                      <AiOutlineEye className="mr-1" /> View
                    </button>
                    <button className={`flex-1 bg-yellow-500 hover:bg-yellow-600 text-white px-2 py-1.5 rounded flex items-center justify-center transition-colors text-sm`}>
              ...
              <div className="col-span-full text-center py-8"><p className="text-gray-500 text-lg">No expired bureaus found.</p></div>
            )}
          </div>
