...
router.get('/testimonials/:id', (req, res) => {
  ...
  const selectQuery = 'SELECT id, name, position, content, photo_url as photoUrl FROM testimonials WHERE id = ?';
...
router.post('/testimonials-adding', upload.single('photo'), (req, res) => {
  ...
  const insertQuery = 'INSERT INTO testimonials (name, position, content, photo_url, created_at) VALUES (?, ?, ?, ?, NOW())';
...
router.put('/testimonials-update', upload.single('photo'), (req, res) => {
  ...
  db.query('SELECT photo_url FROM testimonials WHERE id = ?', [id], (err, result) => {
    if (err) {
      console.error('Database error:', err);
      return res.status(500).json({ message: 'Server error. Please try again later.' });
    }
    ...
    const updateQuery = 'UPDATE testimonials SET name = ?, position = ?, content = ?, photo_url = ? WHERE id = ?';
...
router.delete('/testimonials-delete/:id', (req, res) => {
  const { id } = req.params;
