...
router.get('/testimonials1/:id', (req, res) => {
  const { id } = req.params;

  const selectQuery = 'SELECT id, name, position, content, photo_url as photoUrl FROM distrinutorreviews WHERE id = ?';
...
router.post('/testimonials1-adding', upload.single('photo'), (req, res) => {
  ...
  const insertQuery = 'INSERT INTO distrinutorreviews (name, position, content, photo_url, created_at) VALUES (?, ?, ?, ?, NOW())';
...
router.put('/testimonials1-update', upload.single('photo'), (req, res) => {
  ...
  db.query('SELECT photo_url FROM distrinutorreviews WHERE id = ?', [id], (err, result) => {
    if (err) {
      console.error('Database error:', err);
      return res.status(500).json({ message: 'Server error. Please try again later.' });
    }
    ...
    const updateQuery = 'UPDATE distrinutorreviews SET name = ?, position = ?, content = ?, photo_url = ? WHERE id = ?';
    db.query(updateQuery, [name, position, content, photoUrl, id], (err, result) => {
      ...
      res.status(200).json({ 
        message: 'distrinutorreviews updated successfully!',
        photoUrl
      });
    });
  });
...
router.delete('/testimonials1-delete/:id', (req, res) => {
  const { id } = req.params;
  ...
  db.query('SELECT photo_url FROM distrinutorreviews WHERE id = ?', [id], (err, result) => {
    ...
    const deleteQuery = 'DELETE FROM distrinutorreviews WHERE id = ?';
    db.query(deleteQuery, [id], (err, result) => {
      ...
      res.status(200).json({ message: 'distrinutor reviews deleted successfully!' });
    });
  });
});
